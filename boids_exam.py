...
class Boid:
    ...
    def _clampSpeed(self) -> None:
         ...
         if speed > config.BOID_SPEED_MAX:
            self.vx = (self.vx / speed) * config.BOID_SPEED_MAX
            self.vy = (self.vy / speed) * config.BOID_SPEED_MAX
         elif speed < config.BOID_SPEED_MIN:
            self.vx = (self.vx / speed) * config.BOID_SPEED_MIN
            self.vy = (self.vy / speed) * config.BOID_SPEED_MIN
    ...
    def update(self, boids: List['Boid'], dt: int) -> None:
        ...
        if config.SEPARATION_ON:
            sep = self._separation(boids) * config.SEPARATION_STEER_STRENGTH
            self.vx += sep.x; self.vy += sep.y
        
        if config.ALIGNEMENT_ON:
            ali = self._alignment(boids) * config.ALIGNEMENT_STEER_STRENGTH
            self.vx += ali.x; self.vy += ali.y
            
        if config.COHESION_ON:
            coh = self._cohesion(boids) * config.COHESION_STEER_STRENGTH
            self.vx += coh.x; self.vy += coh.y
